...
if (!is.numeric(rep_x) || length(rep_x) != 1 || rep_x < 1) {
  stop("rep_x must be a positive number", call. = FALSE)
}
...
if (is.null(fut5_result) || length(fut5_result) < 3) {
...
LLB_B <- if (!is.null(fut5_result[[2]])) fut5_result[[2]] else stop("Missing LLB_B from fut5", call. = FALSE)
res4_ja <- if (!is.null(fut5_result[[3]])) fut5_result[[3]] else ""
